...
- 📝 **Улучшенная обработка ошибок** - Подробные логи и подсказки при проблемах с отправкой сообщений
...
sequenceDiagram
    ...
    participant API as 🌐 MAX API
    participant DB as 💾 База данных

    User->>MAX: Отправляет команду /start
    MAX->>Webhook: POST /webhook/:token<br/>(event: message_created)
    Webhook->>Bot: handleUpdate(event)
    Bot->>DB: Сохранить/обновить пользователя
    Bot->>API: POST /messages<br/>(chat_id, text, attachments)
    API-->>Bot: Ответ с результатом
    Bot->>Webhook: Логирование результата
    Webhook->>MAX: HTTP 200 OK
    MAX->>User: Показывает сообщение от бота
    
    Note over User,DB: Обработка callback-запросов
    User->>MAX: Нажимает inline-кнопку
    MAX->>Webhook: POST /webhook/:token<br/>(event: message_callback)
    Webhook->>Bot: handleCallbackQuery(event)
    Bot->>DB: Получить данные пользователя
    Bot->>API: POST /messages<br/>(ответ на callback)
    API-->>Bot: Ответ с результатом
    MAX->>User: Показывает новое сообщение
```
...
```mermaid
flowchart TD
    A[Начало настройки] --> B[Создать .env файл]
    B --> C[Получить BOT_TOKEN на dev.max.ru]
    C --> D{Локальная<br/>разработка?}
    
    D -->|Да| E[Установить ngrok]
    E --> F[Запустить ngrok http 3000]
    F --> G[Скопировать HTTPS URL]
    G --> H[Обновить WEBHOOK_URL в .env]
    
    D -->|Нет| I[Развернуть на сервере]
    I --> J[Указать WEBHOOK_URL на домен]
    
    H --> K[Установить зависимости<br/>npm install]
    J --> K
    K --> L[Проверить подключение<br/>npm run test-bot]
    L --> M{Токен<br/>валидный?}
    
    M -->|Нет| C
    M -->|Да| N[Запустить бота<br/>npm run dev]
    N --> O[Настроить webhook<br/>в панели MAX]
    O --> P[Проверить работу<br/>отправить /start]
    P --> Q{Бот<br/>отвечает?}
    
    Q -->|Нет| R[Проверить логи]
    R --> S[Исправить ошибки]
    S --> N
    Q -->|Да| T[✅ Бот готов к работе]
...
```mermaid
flowchart TD
    A[Webhook получает событие] --> B{Тип события}
    
    B -->|message_created| C[handleMessage]
    B -->|message_callback| D[handleCallbackQuery]
    B -->|callback_query| D
    B -->|bot_started| E[handleStart]
    B -->|bot_stopped| F[Логирование]
    B -->|Другое| G[Логирование неизвестного события]
    
    C --> H{Команда<br/>или текст?}
    H -->|Команда /start| E
    H -->|Команда /catalog| I[handleCatalog]
    H -->|Команда /create| J[handleCreateRequest]
    H -->|Команда /profile| K[handleProfile]
    H -->|Текст кнопки| L[Обработка текста кнопки]
    H -->|Обычный текст| M{Есть состояние<br/>пользователя?}
    
    M -->|Да, создание заявки| N[handleRequestCreationStep]
    M -->|Нет| E
    
    D --> O{Тип callback}
    O -->|view_request| P[handleViewRequest]
    O -->|respond_request| Q[handleRespondToRequest]
    O -->|category| R[Обновить состояние]
    O -->|type| R
    O -->|region| R
    O -->|confirm| S[Создать заявку]
    O -->|cancel| T[handleCancel]
    O -->|page| I
    O -->|filter| U[handleFilter]
    
    I --> V[Отправить сообщение<br/>с клавиатурой]
    J --> V
    K --> V
    P --> V
    Q --> V
    ...
    W --> X{Успешно?}
    X -->|Да| Y[✅ Сообщение отправлено]
    X -->|Нет| Z[Логирование ошибки<br/>с подсказками]
...
```mermaid
flowchart LR
    A[Вызов sendMessage] --> B[Формирование body]
    B --> C[Добавление format]
    C --> D[Добавление attachments]
    D --> E{Попытка 1:<br/>/messages}
    E -->|404| F{Попытка 2:<br/>/bot/messages}
    F -->|404| G{Попытка 3:<br/>/v1/messages}
    G -->|404| H[❌ Все endpoint'ы<br/>не найдены]
    E -->|200| I[✅ Успех]
    F -->|200| I
    G -->|200| I
    E -->|401| J[❌ Ошибка авторизации<br/>💡 Проверить токен]
    E -->|400| K[❌ Ошибка формата<br/>💡 Проверить body]
    F -->|401| J
    F -->|400| K
    G -->|401| J
    G -->|400| K
...
2. **Проверьте логи:**
   ...
   npm run dev
   ```
   Ищите сообщения с префиксом `❌` (ошибки) или `💡` (подсказки)
...
- **Проверьте логи:** При получении события от MAX в логах появится сообщение `📨 ===== ПОЛУЧЕНО СОБЫТИЕ ОТ MAX =====`
...
- ✅ Добавлена обработка callback-запросов (нажатия на inline-кнопки)
- ✅ Улучшена обработка ошибок с подробными подсказками
- ✅ Добавлена автоматическая поддержка разных форматов API endpoint'ов
